...
object Movie extends App {
  ...
  object MoviesProtocol extends DefaultJsonProtocol {
    ...
    implicit val nameFormat = jsonFormat4(Name)
    implicit val ratingFormat = jsonFormat2(Rating)
    implicit val principalFormat = jsonFormat2(Principal)
    implicit val formatFormat = jsonFormat4(Format)
    implicit val productionFormat = jsonFormat4(Production)
    implicit val reviewsFormat = jsonFormat7(Reviews)
    implicit val movieFormat = jsonFormat11(Movie)
  }
  ...
  def testSerializationAndDeserialization(ms: Seq[Movie]): Boolean = {
    ...
    val json = ms.toJson
    val deserialized = json.convertTo[Seq[Movie]]
    deserialized == ms
  }
